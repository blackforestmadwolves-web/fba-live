...
function onOpen() {
  SpreadsheetApp.getUi().createMenu('FBA App')
    ...
    .addSeparator()
    .addItem('Projections: Import-Tabelle vorbereiten', 'setupProjectionConsensus')
    .addItem('Projections: Quellen aktualisieren', 'refreshProjectionConsensus')
    .addToUi();
}
function testData() {
  var d = buildData();
...
function validateProjectionTotalsV36_(projectedGp, totals) {
  ...
  for (var i = 0; i < FBA_PROJECTION_ENGINE_V36.projectionStats.length; i++) {
    var raw = totals[FBA_PROJECTION_ENGINE_V36.projectionStats[i]];
...
function buildMonsterPayloadV29_(requestedWeek) {
  ...
  return {
    ...
    sourceStatus:[
      ...
      {id:'fantasypros',label:'FantasyPros – aktuelle Projections noch nicht bestätigt',active:false},
      {id:'hashtag',label:'Hashtag Basketball – vollständiger Import noch nicht bestätigt',active:false}
    ]
  };
}
...
function buildMonsterPayloadV30_(requestedWeek,force) {
  ...
  var projectionEngine=buildProjectionEnginePayloadV36_(nbaSchedule,nbaSeasonSchedule);
  try{projectionEngine=applyProjectionConsensusV44_(projectionEngine);}catch(consensusError){projectionEngine.active=false;projectionEngine.status='WAITING_EXPERT_PROJECTIONS';projectionEngine.baseline=Object.assign({},projectionEngine.baseline,{active:false,status:'WAITING_EXPERT_PROJECTIONS'});projectionEngine.consensus={version:44,status:'ERROR',appliedPlayers:0,sources:FBA_CONSENSUS_V44.sources.map(function(s){return {id:s.id,name:s.name,state:'ERROR',rows:0,reason:'Expert-Projektionen konnten nicht geladen werden.'};}),message:'Expert-Projektionen konnten nicht geladen werden; aktuelle Projektionswerte bleiben offen.'};}
  var projectionWaiting=projectionEngine.baseline.feedStatus==='WAITING_ESPN_PROJECTIONS';
  overlayProjectionEventStatusV36_(nbaSchedule,nbaSeasonSchedule,projectionEngine);
  return {ok:true,version:44,generated:new Date().toISOString(),currentMatchupPeriod:Number(projectionEngine.actual&&projectionEngine.actual.currentMatchupPeriod||0),roster:compactRoster,espnFantasyPositions:fantasyPositions,espnPlayerPool:espnPlayerPool,adpTrend:adpPayload,schedule:schedule,nbaSchedule:nbaSchedule,nbaSeasonSchedule:nbaSeasonSchedule,projectionEngine:projectionEngine,
...
function matchupMonsterResponseV30_(p) {
  ...
  if(action==='projections_refresh'){
    if(!validMonsterDeviceV29_(p.token||''))return monsterJsonResponseV29_({ok:false,error:'Gerät nicht freigeschaltet.',locked:true},p.callback);
    try{return monsterJsonResponseV29_({ok:true,consensus:refreshProjectionConsensusV44_(true)},p.callback);}catch(err){return monsterJsonResponseV29_({ok:false,error:String(err.message||err)},p.callback);}
  }
  if(action==='refresh'){
    if(!validMonsterDeviceV29_(p.token||''))return monsterJsonResponseV29_({ok:false,error:'Gerät nicht freigeschaltet.',locked:true},p.callback);
 ...
 * Additive layer; v36 actual/ownership/schedule contract stays unchanged.
 * No source rows enter buildData()/public data=1 or the public repository.
 */
var FBA_CONSENSUS_V44 = {
  inputs:'FBA_Projection_Inputs', snapshots:'FBA_Projection_Snapshots', baseline:'FBA_Consensus_Baseline',
  statusKey:'FBA_CONSENSUS_STATUS_V44', freezeKey:'FBA_CONSENSUS_FROZEN_V44', intervalMs:86400000,
  sources:[
    ...
    {id:'hashtag',name:'Hashtag Basketball',family:'hashtag',route:'import',reason:'Angemeldete 2026/27-Projektionen über die FBA Browser Bridge.'},
    ...
    {id:'basketballreference',name:'Basketball-Reference',family:'bref',route:'import',reason:'Serverabruf HTTP 403; pro 36 Minuten, benötigt belegte Minuten und GP.'}
  ]
};
var FBA_CONSENSUS_INPUT_HEADERS_V44 = ['source_id','season_id','player_id','full_name','basis','projected_gp','projected_mpg','snapshot_date','source_url','origin_family']
  .concat(FBA_PROJECTION_ENGINE_V36.projectionStats).concat(['provider_player_id','provider_name','observed_at','provider_updated_at']);
...
var FBA_EXPERT_POLICY_V71 = {version:93,seasonId:2027,confirmedSources:['espn','cbs','lineupexperts','hashtag']};
...
function expertSnapshotInputV71_(row) {
  return Object.assign({},row.base,{source_id:row.sourceId,season_id:row.seasonId,player_id:row.id,full_name:row.name,basis:'per_game',projected_gp:row.projectedGp,snapshot_date:row.snapshotDate,source_url:row.sourceUrl,origin_family:row.family,provider_player_id:row.providerPlayerId,provider_name:row.providerName,observed_at:row.observedAt,provider_updated_at:row.providerUpdatedAt,position_lists:row.positionLists});
}
function consensusNumberV44_(value) {
  if (typeof value==='number') return isFinite(value)&&value>=0 ? value : null;
  if (typeof value!=='string'||!value.trim()||/^(?:-|—|U|N\/A|null)$/i.test(value.trim())) return null;
  var s=value.trim();
...
function consensusNameV44_(value) {
  return String(value||'').normalize('NFD').replace(/[\u0300-\u036f]/g,'').toLowerCase().replace(/[^a-z0-9]/g,'');
}
function consensusIdentityIndexV44_(metadata) {
  var ids={},names={};
  (metadata||[]).forEach(function(p){
    if(p.season_id && Number(p.season_id)!==Number(ESPN_SYNC_V1.seasonId))return;
    var id=String(p.player_id||p.id||''),name=String(p.full_name||p.fullName||'');if(!id||!name)return;
    ids[id]={id:id,name:name};var key=consensusNameV44_(name);names[key]=names[key]||[];
    if(names[key].indexOf(id)<0)names[key].push(id);
  });return {ids:ids,names:names};
}
function normalizeConsensusRowV44_(input,index,now) {
  var source=consensusSourceV44_(String(input.source_id||'')),season=Number(input.season_id),id=String(input.player_id||'');
  function reject(reason){return {ok:false,reason:reason};}
  if(!source)return reject('UNKNOWN_SOURCE');
  if(season!==Number(ESPN_SYNC_V1.seasonId))return reject('WRONG_SEASON');
  var family=source.family;
  if(!family){
    // An aggregate with unknown lineage cannot become an extra expert vote.
    if(!/^(espn|rotowire|cbs|lineupexperts|hashtag|fanscout|bref)$/.test(String(input.origin_family||'')))return reject('UNKNOWN_LINEAGE');
    family=String(input.origin_family);
  }
  var identity=index.ids[id],named=consensusNameV44_(input.full_name);
  if(id&&!identity)return reject('UNKNOWN_PLAYER_ID');
  if(identity&&named&&consensusNameV44_(identity.name)!==named)return reject('PLAYER_ID_NAME_CONFLICT');
  if(!identity){var matches=index.names[named]||[];if(matches.length!==1)return reject(matches.length?'AMBIGUOUS_PLAYER':'UNMATCHED_PLAYER');identity=index.ids[matches[0]];}
  var date=consensusDateV44_(input.snapshot_date),stamp=Date.parse(date+'T00:00:00Z'),today=Date.parse(String(now).slice(0,10)+'T23:59:59Z');
  if(!/^\d{4}-\d{2}-\d{2}$/.test(date)||!isFinite(stamp)||new Date(stamp).toISOString().slice(0,10)!==date||stamp>today||today-stamp>32*86400000)return reject('STALE_OR_INVALID_DATE');
  var gp=consensusNumberV44_(input.projected_gp),minutes=consensusNumberV44_(input.projected_mpg),basis=String(input.basis||''),base={};
  if(!gp||gp>82)return reject('INVALID_GP');
  if(['per_game','totals','per_36'].indexOf(basis)<0)return reject('INVALID_BASIS');
  if(basis==='per_36'&&(!minutes||minutes>48))return reject('MISSING_PROJECTED_MINUTES');
  FBA_PROJECTION_ENGINE_V36.projectionStats.forEach(function(key){var v=consensusNumberV44_(input[key]);base[key]=v==null?null:basis==='totals'?v/gp:basis==='per_36'?v*minutes/36:v;});
  if(base.FGM!=null&&base.FGA!=null&&base.FGM>base.FGA||base.FTM!=null&&base.FTA!=null&&base.FTM>base.FTA||base['3PM']!=null&&base.FGM!=null&&base['3PM']>base.FGM)return reject('INVALID_SHOOTING');
  // A percentage without its shot volume is never manufactured into attempts.
  if(base.FGM==null||base.FGA==null){base.FGM=null;base.FGA=null;}
  if(base.FTM==null||base.FTA==null){base.FTM=null;base.FTA=null;}
  ...
  return {ok:true,row:{id:identity.id,name:identity.name,sourceId:source.id,family:family,seasonId:season,projectedGp:gp,base:base,snapshotDate:date,sourceUrl:String(input.source_url||''),basis:basis,
    providerPlayerId:String(input.provider_player_id||''),providerName:String(input.provider_name||''),observedAt:String(input.observed_at||''),providerUpdatedAt:input.provider_updated_at?consensusDateV44_(input.provider_updated_at):null,
...
function mergeConsensusV44_(rows) {
  ...
  return Object.keys(byPlayer).sort().map(function(id){
    var candidates=byPlayer[id].slice().sort(function(a,b){return b.snapshotDate.localeCompare(a.snapshotDate)||(a.sourceId==='fantasypros'?1:0)-(b.sourceId==='fantasypros'?1:0)||a.sourceId.localeCompare(b.sourceId);}),fields={},base={},used={},gpFamilies={};
    FBA_PROJECTION_ENGINE_V36.projectionStats.forEach(function(key){
      var families={},contributions=[];
      candidates.forEach(function(row){if(row.base[key]==null||families[row.family])return;families[row.family]=true;contributions.push({sourceId:row.sourceId,family:row.family,value:row.base[key],date:row.snapshotDate});used[row.sourceId]=true;});
      fields[key]=contributions;base[key]=contributions.length?contributions.reduce(function(s,r){return s+r.value;},0)/contributions.length:null;
    });
    var gps=[];candidates.forEach(function(row){if(!used[row.sourceId]||gpFamilies[row.family])return;gpFamilies[row.family]=true;gps.push(row.projectedGp);});
    var meanGp=gps.length?gps.reduce(function(a,b){return a+b;},0)/gps.length:0,complete=Object.keys(base).every(function(k){return base[k]!=null;});
    // Keep the integer-GP v36 contract; preserve the unrounded mean for audit.
    var gp=Math.max(1,Math.min(82,Math.round(meanGp)));
    if(complete&&(base.FGM>base.FGA||base.FTM>base.FTA||base['3PM']>base.FGM))complete=false;
    var sources=Object.keys(used).sort(),families=Object.keys(gpFamilies);
    return {id:id,name:candidates[0].name,seasonId:Number(ESPN_SYNC_V1.seasonId),base:base,projectedGp:gp,meanProjectedGp:meanGp,complete:complete,
      sourceIds:sources,sourceCount:sources.length,independentCount:families.length,fields:fields,sourceDates:candidates.filter(function(r){return used[r.sourceId];}).map(function(r){return {sourceId:r.sourceId,date:r.snapshotDate};}),
      gpPolicy:'Gerundeter Mittelwert der unabhängigen Quellen; Rohmittelwert separat gespeichert.'};
  });
}
function consensusHtmlTextV44_(html) {
  return String(html||'').replace(/<script\b[\s\S]*?<\/script>/gi,'').replace(/<style\b[\s\S]*?<\/style>/gi,'').replace(/<[^>]*>/g,' ').replace(/&nbsp;|&#160;/gi,' ').replace(/&amp;/gi,'&').replace(/&#(\d+);/g,function(_,n){return String.fromCharCode(Number(n));}).replace(/&quot;/g,'"').replace(/&#39;|&apos;/g,"'").replace(/\s+/g,' ').trim();
}
function parseYahooConsensusV44_(html,stamp) {
  if(!/<option[^>]*value=["']S_PSR["'][^>]*selected/i.test(html)&&!/<option[^>]*selected[^>]*value=["']S_PSR["']/i.test(html))return {rows:[],status:'WRONG_VIEW'};
  var season=String(Number(ESPN_SYNC_V1.seasonId)-1)+'-'+String(ESPN_SYNC_V1.seasonId).slice(-2);
  if(html.indexOf(season)<0)return {rows:[],status:'WRONG_SEASON'};
  var table=(String(html).match(/<table\b[^>]*[\s\S]*?<\/table>/i)||[])[0]||'',trs=table.match(/<tr\b[\s\S]*?<\/tr>/gi)||[],header=[],rows=[];
  var aliases={'GP*':'projected_gp','GP':'projected_gp','PTS':'PTS','REB':'REB','AST':'AST','ST':'STL','STL':'STL','BLK':'BLK','3PTM':'3PM','FGM/A':'FG_PAIR','FGM/A*':'FG_PAIR','FTM/A':'FT_PAIR','FTM/A*':'FT_PAIR','FGM':'FGM','FGA':'FGA','FTM':'FTM','FTA':'FTA'};
  trs.forEach(function(tr){
    var cells=tr.match(/<t[hd]\b[\s\S]*?<\/t[hd]>/gi)||[];
    if(/<th\b/i.test(tr)){var h=cells.map(function(c){return consensusHtmlTextV44_(c).replace(/[\ue000-\uf8ff]/g,'').trim();});if(h.indexOf('PTS')>=0)header=h;return;}
    if(!header.length||cells.length!==header.length)return;
    var playerCell=cells[header.indexOf('Players')]||'',link=playerCell.match(/<a\b[^>]*class=["'][^"']*name[^"']*["'][^>]*>([\s\S]*?)<\/a>/i);
    if(!link)link=playerCell.match(/<a\b[^>]*href=["'][^"']*\/nba\/players\/\d+[^"']*["'][^>]*>([\s\S]*?)<\/a>/i);
    if(!link)return;
    var row={source_id:'yahoo',season_id:ESPN_SYNC_V1.seasonId,full_name:consensusHtmlTextV44_((link[0].match(/title=["']([^"']+)["']/i)||[])[1]||link[1]),basis:'totals',snapshot_date:stamp.slice(0,10),source_url:'https://basketball.fantasysports.yahoo.com/'};
    cells.forEach(function(c,i){var key=aliases[header[i]],value=consensusHtmlTextV44_(c);if(!key)return;if(/_PAIR$/.test(key)){var pair=value.split('/');if(pair.length===2){row[key==='FG_PAIR'?'FGM':'FTM']=pair[0];row[key==='FG_PAIR'?'FGA':'FTA']=pair[1];}}else row[key]=value;});rows.push(row);
  });return {rows:rows,status:rows.length?'PARSED':'NO_ROWS'};
}
function consensusStoredRowsV44_(sheet) {
  return sheetObjectsV2_(sheet).filter(function(r){return Number(r.season_id)===Number(ESPN_SYNC_V1.seasonId);}).map(function(r){try{return JSON.parse(r.payload_json);}catch(e){return null;}}).filter(Boolean);
}
function ensureConsensusSheetsV44_() {
  var inputSheet=ensureSimpleEspnSheetV1_(FBA_CONSENSUS_V44.inputs,FBA_CONSENSUS_INPUT_HEADERS_V44);
  if(inputSheet.getRange&&inputSheet.getRange(1,FBA_CONSENSUS_INPUT_HEADERS_V44.length).getValue()!=='provider_updated_at')inputSheet.getRange(1,1,1,FBA_CONSENSUS_INPUT_HEADERS_V44.length).setValues([FBA_CONSENSUS_INPUT_HEADERS_V44]);
  ensureSimpleEspnSheetV1_(FBA_CONSENSUS_V44.snapshots,FBA_CONSENSUS_SNAPSHOT_HEADERS_V44);
  ensureSimpleEspnSheetV1_(FBA_CONSENSUS_V44.baseline,FBA_CONSENSUS_BASELINE_HEADERS_V44);
  return {ok:true,inputSheet:FBA_CONSENSUS_V44.inputs};
}
function consensusStatusV44_() {
  try{return JSON.parse(espnPropertiesV1_().getProperty(FBA_CONSENSUS_V44.statusKey)||'{}');}catch(e){return {};}
}
...
// ESPN omits some statistics entirely. Preserve those as missing so a partial
// source can contribute its supplied fields without inventing zeroes.
function parseEspnConsensusRowsV93_(hub,stamp) {
  if(Number((hub||{}).seasonId)!==Number(ESPN_SYNC_V1.seasonId))return [];
  var byId={};
  (hub.players||[]).forEach(function(entry){
    var player=rawFantasyPlayerV36_(entry),id=String(player.id||'');if(!id)return;
    projectionStatRowsV36_(player).forEach(function(stat){
      if(Number(stat.seasonId)!==Number(ESPN_SYNC_V1.seasonId)||Number(stat.statSourceId)!==1||Number(stat.statSplitTypeId)!==0)return;
      var gp=projectionNumberV36_(stat.stats,FBA_PROJECTION_ENGINE_V36.projectionStatIds.GP);
      if(!gp||gp>82)return;
      var row={source_id:'espn',season_id:ESPN_SYNC_V1.seasonId,player_id:id,full_name:player.fullName,projected_gp:gp,basis:'totals',snapshot_date:stamp.slice(0,10),observed_at:stamp,source_url:'https://fantasy.espn.com/basketball/players/projections'};
      FBA_PROJECTION_ENGINE_V36.projectionStats.forEach(function(k){row[k]=projectionNumberV36_(stat.stats,FBA_PROJECTION_ENGINE_V36.projectionStatIds[k]);});
      if(!byId[id]||row.projected_gp>byId[id].projected_gp)byId[id]=row;
    });
  });
...
function refreshProjectionConsensusV44_(force,importsOnly) {
  ...
  if(!previous.importsDirty&&!emptyImportBaseline&&previous.policyVersion===FBA_EXPERT_POLICY_V71.version&&previous.lastAttempt&&Date.parse(stamp)-Date.parse(previous.lastAttempt)<(force?15*60000:FBA_CONSENSUS_V44.intervalMs))return previous;
  var lock=LockService.getScriptLock();if(!lock.tryLock(1000))return previous;
  try{
    ensureConsensusSheetsV44_();
    var actual=aggregateProjectionActualsV36_(sheetObjectsV2_(ESPN_PLAYER_HUB_V2.dailySheet));
    var started=actual.completeGames>0,existing=consensusStoredRowsV44_(FBA_CONSENSUS_V44.baseline).filter(expertConsensusConfirmedV71_),frozen=props.getProperty(FBA_CONSENSUS_V44.freezeKey)==='1'||started;
    if(frozen){props.setProperty(FBA_CONSENSUS_V44.freezeKey,'1');var frozenStatus=Object.assign({},previous,{policyVersion:FBA_EXPERT_POLICY_V71.version,lastAttempt:stamp,frozenAt:previous.frozenAt||stamp,frozen:true,status:existing.length?'FROZEN':'WAITING_PRESEASON_BASELINE',message:'Baseline eingefroren; echte Spiele werden ausschließlich durch die v36-Ist-Kette ersetzt.'});props.setProperty(FBA_CONSENSUS_V44.statusKey,JSON.stringify(frozenStatus));return frozenStatus;}
    var metadata=sheetObjectsV2_(ESPN_PLAYER_HUB_V2.playersSheet),native=[],espnError='';
    // On policy migration, activate newly confirmed ESPN on the first private
    // load. Existing imported providers need no extra network refresh here.
    if((!importsOnly||previous.policyVersion!==FBA_EXPERT_POLICY_V71.version)&&expertSourceConfirmedV71_('espn'))try{
      var hub=fetchEspnFantasyHubV2_();if(Number(hub.seasonId)!==Number(ESPN_SYNC_V1.seasonId))throw new Error('WRONG_SEASON');
      (hub.players||[]).forEach(function(entry){var p=rawFantasyPlayerV36_(entry);if(p&&p.id)metadata.push({player_id:String(p.id),full_name:p.fullName,season_id:hub.seasonId});});
      native=parseEspnConsensusRowsV93_(hub,stamp);
    ...
    FBA_CONSENSUS_V44.sources.forEach(function(source){
      if(!expertSourceConfirmedV71_(source.id)){
        kept=kept.concat(old.filter(function(r){return r.sourceId===source.id;}));
        statuses.push({id:source.id,name:source.name,state:'WAITING_CONFIRMATION',reason:'Für 2026/27 noch nicht als Projektionsquelle bestätigt.',rows:0,completeRows:0,contributing:false});return;
      }
      ...
      if(importsOnly&&!(source.id==='espn'&&native.length)){raw=old.filter(function(r){return r.sourceId===source.id;}).map(expertSnapshotInputV71_);reason='Bestätigter Import mit fester Spielerzuordnung.';}
      if(source.id==='espn'){fetchFailed=Boolean(espnError);state=raw.length?'PARSED':'WAITING_CURRENT_SEASON';reason=espnError||(!raw.length?'Keine gültigen ESPN-Projektionszeilen für 2026/27.':'ESPN Saison-Projections 2026/27 · erwartete Spiele und vollständiges Wurfvolumen. Kein bestätigter ROS-Feed.');}
      if(source.id==='yahoo'&&!importsOnly){
        // Sequential, bounded pagination; stop on any access restriction.
        try{for(var page=0;page<1;page++){
          var response=UrlFetchApp.fetch(source.url+(page*25),{muteHttpExceptions:true,followRedirects:true});
          if(response.getResponseCode()!==200)throw new Error('HTTP '+response.getResponseCode());
          var parsed=parseYahooConsensusV44_(response.getContentText(),stamp);
      ...
      if(source.route==='cbs'&&!importsOnly){
      ...
      if(source.route==='audit'&&!importsOnly){
      ...
      raw=raw.concat(imports.filter(function(r){return r.source_id===source.id;}));
      if(source.id==='cbs'&&importsOnly){
        var bindings=sheetObjectsV2_(FBA_CBS_V70.mapSheet).filter(function(r){return Number(r.season_id)===Number(ESPN_SYNC_V1.seasonId);}),matched=bindings.filter(function(r){return r.status==='MATCHED';}),pending=bindings.filter(function(r){return r.status!=='MATCHED';}),byEspn={};
        matched.forEach(function(r){var id=String(r.player_id);byEspn[id]=byEspn[id]===undefined?r:null;});
        raw=raw.map(function(r){var binding=byEspn[String(r.player_id)];return binding?Object.assign({},r,{provider_player_id:String(binding.provider_player_id),provider_name:binding.provider_name}):r;});
        cbsAudit={mapping:{matched:matched.length,pending:pending.length,issues:pending.slice(0,100).map(function(r){return {name:r.provider_name,providerPlayerId:String(r.provider_player_id),reason:r.status};})},pages:[]};
      }
      var valid=[],rejected={},seen={};raw.forEach(function(r){var result=normalizeConsensusRowV44_(r,index,stamp);if(result.ok){var prior=seen[result.row.id];if(!prior||consensusPreferRowV44_(result.row,prior))seen[result.row.id]=result.row;}else rejected[result.reason]=(rejected[result.reason]||0)+1;});
      ...
      var retainedCbs=0;
      if((source.id==='cbs'||source.id==='espn')&&!fetchFailed&&valid.length&&lost){
      ...
      if(source.id==='espn'){
        sourceStatus.refreshMode='native';sourceStatus.providerIdentityType='espn_id';
        sourceStatus.lastChecked=valid.map(function(r){return r.observedAt||r.snapshotDate;}).sort().pop()||null;
        sourceStatus.providerUpdatedAt=null;sourceStatus.retainedPlayers=retainedCbs;
        sourceStatus.projectionHorizon='season';sourceStatus.rosVerified=false;
      }
    ...
    // Missing/incomplete refreshes must not remove a previous complete player.
    var completeIds={};complete.forEach(function(r){completeIds[r.id]=true;});
    existing.forEach(function(r){if(!completeIds[r.id]&&consensusRecentV44_(r,stamp))complete.push(r);});
    if(complete.length)writeConsensusRowsV44_(FBA_CONSENSUS_V44.baseline,FBA_CONSENSUS_BASELINE_HEADERS_V44,complete.map(function(r){return [ESPN_SYNC_V1.seasonId,r.id,JSON.stringify(r),stamp];}));
    revision=stableHashV36_(JSON.stringify(complete));
    var status={version:44,policyVersion:FBA_EXPERT_POLICY_V71.version,confirmedSources:FBA_EXPERT_POLICY_V71.confirmedSources.slice(),lastAttempt:stamp,lastSuccess:complete.length&&revision!==previous.revision?stamp:previous.lastSuccess||null,status:complete.length?'READY':'PARTIAL',frozen:false,revision:revision,sources:statuses,players:merged.length,completePlayers:complete.length};
    props.setProperty(FBA_CONSENSUS_V44.statusKey,JSON.stringify(status));return status;
...
function applyProjectionConsensusV44_(engine) {
  ...
  // Every provider, including ESPN, must pass the same consensus validation.
  // Do not leak an older native baseline around freshness/identity checks.
  (engine.players||[]).forEach(function(p){map[String(p.id)]=Object.assign({},p,{base:null,baseline:null,projectedGp:null,seasonFinish:null,consensus:null});});
  if(engine.actual&&engine.actual.completeGames>0&&rows.length&&!status.frozen){status.frozen=true;status.frozenAt=new Date().toISOString();espnPropertiesV1_().setProperty(FBA_CONSENSUS_V44.freezeKey,'1');espnPropertiesV1_().setProperty(FBA_CONSENSUS_V44.statusKey,JSON.stringify(status));}
  var applied=0;rows.forEach(function(row){
    if(!row.complete||Number(row.seasonId)!==Number(ESPN_SYNC_V1.seasonId))return;
    if(!consensusRecentV44_(row,status.frozen?(status.frozenAt||status.lastAttempt):new Date().toISOString()))return;
    var meta=metadata[row.id]||{},old=map[row.id],p=old||{id:row.id,playerId:row.id,name:row.name,nba:nbaAbbreviationV3_(meta.nba_team_id),nbaTeam:nbaAbbreviationV3_(meta.nba_team_id),fantasyPositions:meta.fantasy_positions||meta.primary_position||'',primaryPosition:meta.primary_position||'',injuryStatus:meta.injury_status||'',actual:{gp:0,totals:emptyProjectionStatsV36_(),byWeek:{}}};
    p=Object.assign({},p,{base:row.base,baseline:row.base,projectedGp:row.projectedGp,consensus:row});p.seasonFinish=replaceProjectionWithActualsV36_(p.projectedGp,p.base,p.actual.gp,p.actual.totals);map[row.id]=p;applied++;
  });
  engine.players=Object.keys(map).sort().map(function(id){return map[id];});
  ...
  else{engine.active=false;engine.status='WAITING_EXPERT_PROJECTIONS';engine.baseline=Object.assign({},engine.baseline,{active:false,status:'WAITING_EXPERT_PROJECTIONS',source:'Expert-Projektion 2026/27',count:0});}
  ...
  engine.consensus.projectionHorizon='season';
  engine.consensus.rosReady=false;
  engine.consensus.rosIssue='Aktuelle ROS-Projections fehlen. Die gemeinsame Saison-Projection steht für die Draft-Vorbereitung bereit; Simulation und Matchup-Prognosen benötigen bestätigte Rest-of-Season-Werte.';
  if(!applied){
...
function consensusRecentV44_(row,stamp){return Array.isArray(row.sourceDates)&&row.sourceDates.length>0&&row.sourceDates.every(function(s){var age=Date.parse(stamp)-Date.parse(s.date+'T00:00:00Z');return isFinite(age)&&age>=0&&age<32*86400000;});}

function consensusDateV44_(value){if(Object.prototype.toString.call(value)==='[object Date]'){if(!isFinite(value.getTime()))return '';return Utilities.formatDate(value,book().getSpreadsheetTimeZone(),'yyyy-MM-dd');}return String(value||'').slice(0,10);}
...
/* Hashtag: authenticated browser table -> persistent provider IDs -> private consensus. */
var FBA_HASHTAG_V75={mapSheet:'FBA_Hashtag_Player_Map',statusKey:'FBA_HASHTAG_SYNC_V75',url:'https://hashtagbasketball.com/import-v4/fantasy-basketball-projections'};
// Provider-specific spellings checked against the current ESPN player identities.
var FBA_HASHTAG_NAMES_V75={
  '120000':{name:'Alexandre Sarr',id:'5160992',espn:'Alex Sarr'},
  '17252':{name:'Nicolas Claxton',id:'4278067',espn:'Nic Claxton'},
  '120010':{name:'Ron Holland II',id:'4683771',espn:'Ronald Holland II'}
};
function parseHashtagSnapshotV75_(data,stamp){
  function fail(message){throw new Error('Hashtag: '+message);}
  if(!data||data.sourceUrl!==FBA_HASHTAG_V75.url||data.heading!=='Fantasy Basketball Projections')fail('Projektionsseite nicht bestätigt.');
  var opts=data.options||{},required={DDSHOW:'900',DDGAMES:'0',DropDownList1:'Off',DDPOS:'All',DDTSUM:'All',DDDURATION:'0',DDRANK:'AVG'};
  Object.keys(required).forEach(function(k){if(!opts[k]||opts[k].value!==required[k])fail('Vollständige Pro-Spiel-Ansicht erforderlich ('+k+').');});
  if(opts.DDDURATION.label!=='2026-27 Rest of Season Projections')fail('Falsche Saison oder BLEND/STREAM-Ansicht.');
  var months=['January','February','March','April','May','June','July','August','September','October','November','December'];
  var dm=String(data.updatedText||'').match(/^Updated:\s*(\d{1,2}) ([A-Za-z]+) (\d{4})$/),updated=dm&&dm[3]+'-'+('0'+(months.indexOf(dm[2])+1)).slice(-2)+'-'+('0'+dm[1]).slice(-2);
  if(!updated||!isFinite(Date.parse(updated))||new Date(updated).toISOString().slice(0,10)!==updated||updated>stamp.slice(0,10))fail('Veröffentlichungsdatum ungültig.');
  var h=data.headers,keys=['PLAYER','GP','MPG','FG%','FT%','3PM','PTS','TREB','AST','STL','BLK'];
  if(!Array.isArray(h)||keys.some(function(k){return h.filter(function(v){return v===k;}).length!==1;}))fail('Tabellenspalten geändert.');
  if(!Array.isArray(data.rows)||data.rows.length<100||data.rows.length>900)fail('Gesamtliste unvollständig.');
  var ids={},names={};
  return data.rows.map(function(r){
    var match=String(r.playerLink||'').match(/^\/(\d+)\/player$/),name=String(r.playerName||'').trim();
    if(!match||!name||name.length>100||/^[=+@\-]/.test(name)||!Array.isArray(r.cells)||r.cells.length!==h.length)fail('Spielerzeile ungültig.');
    var id=match[1];if(ids[id]||names[consensusNameV44_(name)])fail('Doppelte Spielerkennung.');ids[id]=true;names[consensusNameV44_(name)]=true;
    function val(k){return String(r.cells[h.indexOf(k)]).trim();}
    function num(k){var n=consensusNumberV44_(val(k));if(n==null)fail('Ungültige Statistik '+name+' / '+k);return n;}
    function shots(k){var m=val(k).match(/^(\d*\.?\d+)\s*\((\d*\.?\d+)\s*\/\s*(\d*\.?\d+)\)$/);if(!m||Number(m[1])>1||Number(m[2])>Number(m[3]))fail('Wurfvolumen fehlt: '+name+' / '+k);return [Number(m[2]),Number(m[3])];}
    var fg=shots('FG%'),ft=shots('FT%');
    return {source_id:'hashtag',season_id:2027,provider_player_id:id,provider_name:name,basis:'per_game',projected_gp:num('GP'),projected_mpg:num('MPG'),
      snapshot_date:stamp.slice(0,10),observed_at:stamp,provider_updated_at:updated,source_url:FBA_HASHTAG_V75.url,origin_family:'hashtag',
      PTS:num('PTS'),REB:num('TREB'),AST:num('AST'),'3PM':num('3PM'),STL:num('STL'),BLK:num('BLK'),FGM:fg[0],FGA:fg[1],FTM:ft[0],FTA:ft[1]};
  });
}
function mapHashtagPlayersV75_(rows,index,stored,stamp){
  var bindings={},duplicates={},used={},updates={},accepted=[],issues=[];
  var suffix=function(n){return consensusNameV44_(String(n).replace(/\s+(?:Jr\.?|Sr\.?|II|III|IV)$/i,''));},names={};
  Object.keys(index.ids).forEach(function(id){var key=suffix(index.ids[id].name);(names[key]=names[key]||[]).push(id);});
  (stored||[]).filter(function(r){return Number(r.season_id)===2027;}).forEach(function(r){var id=String(r.provider_player_id);if(bindings[id])duplicates[id]=true;bindings[id]=r;});
  rows.forEach(function(r){
    var id=r.provider_player_id,old=bindings[id],matches=index.names[consensusNameV44_(r.provider_name)]||names[suffix(r.provider_name)]||[],identity=null,status='';
    var variant=FBA_HASHTAG_NAMES_V75[id],target=variant&&index.ids[variant.id];
    if(!matches.length&&variant&&target&&r.provider_name===variant.name&&target.name===variant.espn)matches=[target.id];
    if(duplicates[id])status='DUPLICATE_MAPPING';
    else if(old&&old.player_id){
      identity=index.ids[String(old.player_id)];
      if(!identity)status='UNKNOWN_PLAYER_ID';
      else if(consensusNameV44_(old.provider_name)!==consensusNameV44_(r.provider_name))status='PROVIDER_NAME_CHANGED';
      else if(matches.length!==1||matches[0]!==identity.id)status='PLAYER_ID_NAME_CONFLICT';
    }else if(matches.length===1)identity=index.ids[matches[0]];
    else status=matches.length?'AMBIGUOUS_PLAYER':'UNMATCHED_PLAYER';
    if(!status&&identity){
      var row=Object.assign({},r,{player_id:identity.id,full_name:identity.name}),check=normalizeConsensusRowV44_(row,index,stamp);
      if(!check.ok)status=check.reason;
      else {if(used[identity.id])throw new Error('Hashtag: Mehrere IDs für denselben ESPN-Spieler.');used[identity.id]=true;accepted.push(row);status='MATCHED';}
    }
    if(status!=='MATCHED')issues.push({providerPlayerId:id,name:r.provider_name,reason:status});
    updates[id]={season_id:2027,provider_player_id:id,player_id:identity?identity.id:String(old&&old.player_id||''),provider_name:r.provider_name,espn_name:identity?identity.name:'',status:status,first_seen:String(old&&old.first_seen||stamp),last_seen:stamp};
  });
  Object.keys(bindings).forEach(function(id){if(!updates[id])updates[id]=bindings[id];});
...
function importHashtagSnapshotV75_(data,cloud){
  ...
  try{
    ...
    if(prior.length&&mapped.rows.length<prior.length*.9)throw new Error('Unerwartet verkürzte Hashtag-Liste; bisherige Werte bleiben erhalten.');
    // Replace this provider only. Missing identities keep their last valid row.
    var newIds={};mapped.rows.forEach(function(r){newIds[r.player_id]=true;});
    var current=inputs.filter(function(r){return Number(r.season_id)===2027&&(r.source_id!=='hashtag'||!newIds[String(r.player_id)]);}).concat(mapped.rows);
    ensureConsensusSheetsV44_();
  ...
  result.merged=false;return result;
}
function doPost(e){
  var p;
  try{
    if(!e||!e.postData||typeof e.postData.contents!=='string'||e.postData.contents.length>650000)throw new Error('Ungültige Anfrage.');
    p=JSON.parse(e.postData.contents);
    if(p.action!=='hashtag_import')throw new Error('Unbekannte Aktion.');
