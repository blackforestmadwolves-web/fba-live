...
(function(root){
  ...
  function stats(player,defaultGames=72){
    if(!player||player.projectionReady===false||player.projectionMissing===true)return null;
    const result={};
  ...
  function rank(rows,team,cat,key='values'){
    ...
    if(!current?.picks||!Number.isFinite(current[key][cat]))return null;
    const eligible=rows.filter(row=>row.picks&&Number.isFinite(row[key][cat]));
  ...
  function analyze(input){
    ...
    const used=new Set(),raw=teams.map(team=>({team,picks:0,missing:0,missingPlayers:[],complete:true,totals:Object.fromEntries(fields.map(f=>[f,0]))}));
    for(const pick of input.picks||[]){
      const row=raw.find(row=>row.team===pick.team),id=String(pick.playerId);
      if(!row||used.has(id))continue;
      used.add(id);row.picks++;
      const player=byId.get(id),s=stats(player,input.defaultGames);
      // A missing projection occupies its roster slot but contributes no stats.
      // Keep stats() strict: unknown free agents must not become recommendations.
      if(!s){row.missing++;row.missingPlayers.push(player?.name||pick.playerName||`Spieler #${id}`);row.complete=false;continue;}
    ...
    const missing=participants.reduce((sum,row)=>sum+row.missing,0),ready=!!own;
    ...
    const needs=active.map(cat=>{
      const peers=participants.filter(row=>row.quality[cat]!=null).map(row=>row.quality[cat]);
    ...
    if(ready&&own&&!full){
      for(const player of pool){
        ...
        const changes=needs.map(need=>{
          let gain;
          if(!own.picks){
            const peers=participants.filter(row=>row.quality[need.cat]!=null).map(row=>row.quality[need.cat]);
