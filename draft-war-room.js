...
(function(root){
  ...
  function coachNote(data){
    if(!data.ready)return {title:'Statistikbasis noch unvollständig',body:`Für ${data.missing} gedraftete Spieler fehlen vollständige Basiswerte. Sobald diese vorliegen, können wir dein Team zuverlässig vergleichen.`};
  ...
  function tilesMarkup(data){
    return data.tiles.map(tile=>{
      ...
      return `<div class="draft-coach-cat ${tone}" data-coach-category="${E(tile.cat)}"><span>${E(tile.cat)}</span><strong>${rankText(tile.rank)}</strong><small>${status}</small></div>`;
    }).join('');
  }
  function markup(){
