...
(function (root) {
  ...
  function preparationBasisMarkup(markup) {
    return markup.replace(/<span class="maik-value-basis">(Expert-Projektion|Ist \+ Rest) 2026\/27<\/span>/g,
      (_, basis) => `<span class="maik-value-basis draft-prep-basis-stack"><span>${basis === 'Expert-Projektion' ? 'Projektion' : 'Ist + Rest'}</span><span>26–27</span></span>`);
  }
  ...
  function cardsMarkup(rows, merged) {
    return rows.map((player, index) => {
      ...
      return preparationBasisMarkup(draftRadarCard(Object.assign({}, player, {rank: index + 1}), index))
  ...
  function pgDraftPreparation() {
    ...
    return `<section class="draft-preparation" aria-label="Draft-Vorbereitung">${sect('Draft-Vorbereitung', 'Dein Blick auf den Draft')}
      ...
      <div id="draft-prep-projections">${root.draftProjectionSourceMarkup?.()||''}</div>
  ...
  function updateView() {
    ...
    const projectionPanel=doc.getElementById('draft-prep-projections');
    if(projectionPanel){const open=projectionPanel.querySelector('details')?.open;projectionPanel.innerHTML=root.draftProjectionSourceMarkup?.()||'';const details=projectionPanel.querySelector('details');if(details)details.open=!!open;}
    if (empty) {
  ...
  const api = Object.freeze({sorts: SORTS, normalizeSort, sortPlayers, mergeValues, filterPlayers, readSort, valueBasisDescription, preparationBasisMarkup});
  if (typeof module !== 'undefined' && module.exports) module.exports = api;
  root.FBA_DRAFT_PREP = api;
  root.pgDraftPreparation = pgDraftPreparation;
  root.draftPreparationRefresh = updateView;
