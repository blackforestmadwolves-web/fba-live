/* ESPN Mock bridge: validate public room metadata and rendered picks. No network or app mutation. */
(function(root,factory){const api=factory();if(typeof module==='object'&&module.exports)module.exports=api;else root.FBA_ESPN_MOCK_CORE=api;})(typeof globalThis!=='undefined'?globalThis:this,function(){
  ...
  function picks(snapshot,config,pool){
    ...
    for(const row of snapshot.picks){
      if(!row||!integer(row.overall,1,104)||typeof row.name!=='string'||row.name.length>160||typeof row.team!=='string'||row.team.length>160)fail('Eine ESPN-Pick-Zeile ist nicht eindeutig lesbar.');
      ...
      const candidates=exact?[byId.get(String(row.playerId))].filter(Boolean):byName.get(normalize(row.name))||[];
      ...
      const result={overall,round:Math.ceil(overall/8),slot:(overall-1)%8+1,team:config.fbaTeams[slot-1],playerId:id,at:''};
  ...
  return {room,metadata,slotFor,teamOrder,picks,changed,normalize};
});
